...
在 Claude Code 的对话里，消息前面加个 `en:` 就能顺手学英语，不用切出去查。

核心：顺手学的摩擦是上下文切换——要停下来把想学的重新组织出来再查，这步就拦住人了。所以让工具直接操作对话里已有的内容。

- `en: 中文` 译成地道英文
- `en:?` 把刚聊的那句拎出来讲
- `en:save` 进个人词典（按词条合并、记遇见次数）

还有个被动模式：学英语的语境里主动指出值得学的表达。自学的难处常常不是学不会，是不知道该学什么，让它主动挑。

## 装一下

代码在 [bencode/daily-skills](https://github.com/bencode/daily-skills)：
